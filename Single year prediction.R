...
x <- read.scsbio(year+1, category = "MSC12", survey = "regular")
x <- x[which(!morphometric.maturity(x)), ]
...
x <- read.scsbio(year, category = "MSC12", survey = "regular")
x <- x[which(!morphometric.maturity(x)), ]
gbarplot(freq(x), border = "grey60", width = 1, ylim = ylim, xlim = xlim, xaxs = "i", grid = TRUE, xaxt = "n")
...
x <- read.scsbio(year+1, category = "M", survey = "regular")
x <- x[which(morphometric.maturity(x)), ]
