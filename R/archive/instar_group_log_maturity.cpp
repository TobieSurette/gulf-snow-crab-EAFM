...
template<class Type> Type objective_function<Type>::operator()(){
   ...
   log_sigma_instar_mature[first_mature_instar - first_instar] = log_sigma_instar[first_mature_instar - first_instar];      
   ...
   for (int i = 0; i < n_group; i++){
      for (int j = 0; j < n_instar; j++){
         ...
         sigma(j,i)        = exp(log_sigma[j]        + log_sigma_instar_group[j * n_group + i]); 
         sigma_mature(j,i) = exp(log_sigma_mature[j] + log_sigma_instar_group[j * n_group + i]); 
      }
   }   
